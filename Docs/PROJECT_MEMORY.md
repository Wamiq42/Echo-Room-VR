...
### BUILD-QUEST-PREFLIGHT-001 — Apply the required Quest build settings

- **Date:** 2026-07-23
- **Goal:** Audit the live Unity project before the first physical Quest build and apply the safe, current Android/XR settings that should not be left for the tester to discover.
- **Result:** The Android target SDK is now explicitly API 34, Active Input Handling is Input System Package only, and this editor's Android texture-compression build subtarget is ASTC. Existing Quest-critical settings were retained: Android active build target, minimum API 32, IL2CPP, ARM64 only, explicit Vulkan with OpenGLES3 fallback, Linear color, Landscape Left, OpenXR initialization, single-pass instanced rendering, Meta XR support/foveation/subsampled layout, Oculus Touch Controller Profile, and the correct two-scene build order.
- **Files modified:**
  - `ProjectSettings/ProjectSettings.asset`
  - `Library/EditorUserBuildSettings.asset` (ignored editor-local setting; ASTC build subtarget)
  - `Docs/PROJECT_MEMORY.md`
- **Files created/moved/deleted:** None.
- **Unity objects affected:** None — Project Settings and editor-local Android build configuration only.
- **Components/assets/settings:** `PlayerSettings.Android.targetSdkVersion` changed from `AndroidApiLevel32` to `AndroidApiLevel34`; `PlayerSettings.activeInputHandler` changed from `2` (Both) to `1` (Input System Package); `EditorUserBuildSettings.androidBuildSubtarget` changed from `Generic` to `ASTC`. Minimum SDK remains API 32. The installed Unity Android SDK contains platforms 34, 35, and 36, so explicit target 34 is locally available. `Assets/XR/XRGeneralSettingsPerBuildTarget.asset` retains `OpenXRLoader` with Initialize XR on Startup. `Assets/XR/Settings/OpenXR Package Settings.asset` retains enabled `MetaXRFeature`, `MetaXRFoveationFeature`, `MetaXRSubsampledLayout`, and `OculusTouchControllerProfile`.
- **Decisions and assumptions:** API 34 was selected instead of Automatic so the project does not unexpectedly move to API 35/36 and because current Meta guidance makes API 34 the safe baseline for newly created dashboard apps. Meta XR SDK 77's local validator still contains an older non-blocking recommendation to target API 32; that stale warning was not followed. Input System-only is safe for this project: the only legacy `Input.*` fallback is compile-guarded in `VRPauseMenu`, and the same path already supports `Keyboard.current`. ASTC is editor-local and therefore is not part of the tracked project commit; another machine should select ASTC before building. The placeholder `DefaultCompany` / `com.DefaultCompany.EchoRoom` identity and debug keystore remain acceptable for local sideload testing but must be replaced with the user's chosen stable identity and persistent signing key before distribution. Hand-tracking features remain disabled because the current tested interaction path is controller-based. Rendering recommendations that could materially alter appearance — HDR, depth texture, SSAO, intermediate texture mode, and MSAA — were not changed blindly before headset profiling.
- **Verification:** Live Unity 6000.3.8f1 readback passed with Android active; API 34 target/API 32 minimum; IL2CPP; ARM64; ASTC; Input System-only; Vulkan then OpenGLES3; OpenXR loader initialized on startup; single-pass instanced rendering; expected Meta/Oculus features; and enabled `MainMenuScene` then `MainScene`. Unity's Android OpenXR project validation returned zero issues. The editor was idle with `EditorUtility.scriptCompilationFailed=False`; a focused final verification returned `PASS`, and the fresh one-minute Console query returned zero Errors. The settings diff contains only the target-SDK and input-handler changes.
- **Known limitations and follow-up:** No APK was produced in this change, so the final merged Android manifest, microphone permission, signing, install, launch, runtime performance, stereo presentation, and physical controller behavior remain `PENDING-HEADSET`. Meta's performance validator still reports non-blocking URP recommendations for depth texture, Intermediate Texture Always, SSAO, 4× MSAA, and HDR; evaluate those with the Quest profiler and visual comparison instead of treating them as build blockers. Before Store submission, choose the final company/package identity, create and securely retain a release keystore, inspect the merged manifest/permissions, and increment Android version codes for releases.

### BUILD-QUEST-STARTUP-INPUT-001 — Correct Quest GameActivity launch and Touch Plus controller input

- **Date:** 2026-07-23
- **Goal:** Fix the first Quest APK's immediate startup failure and restore physical controller input to the UI Toolkit menus on Quest 3/3S.
- **Result:** Android now builds and launches through `UnityPlayerGameActivity`, matching Unity's generated manifest and eliminating the observed `ClassNotFoundException`. The Android OpenXR configuration now enables Meta Quest Touch Plus Controller Profile while retaining Oculus Touch Controller Profile for older supported Quest controllers. Per-CPU APK splitting is disabled so a normal single ARM64 APK path cannot collide with Unity's split-output directory naming.
- **Files modified:**
  - `ProjectSettings/ProjectSettings.asset`
  - `Assets/XR/Settings/OpenXR Package Settings.asset`
  - `Docs/PROJECT_MEMORY.md`
- **Files created during build/verification (ignored build artifacts):**
  - `Builds/Echo Room.arm64-v8a.pre-gameactivity-fix.apk` (preserved original APK with the broken Activity/GameActivity mismatch)
  - `Builds/Echo Room.arm64-v8a.apk/Echo Room.arm64-v8a.apk` (corrected intermediate GameActivity build)
  - `Builds/Echo Room Quest Touch Plus.apk` (final single-APK controller-profile build)
- **Files created and deleted during verification:** `Assets/_EchoRoom/Editor/QuestTouchPlusBuildOnce.cs` and its `.meta` were used only as a guarded one-shot editor build helper, then removed after the successful build.
- **Files moved/deleted as part of the product change:** None.
- **Unity objects affected:** None — this change is project-wide Android/OpenXR configuration only.
- **Unity objects inspected live, read-only:**
  - `Assets/_EchoRoom/Scenes/MainMenuScene.unity :: EventSystem`
  - `Assets/_EchoRoom/Scenes/MainMenuScene.unity :: XR Origin (XR Rig)/Camera Offset/Left Controller`
  - `Assets/_EchoRoom/Scenes/MainMenuScene.unity :: XR Origin (XR Rig)/Camera Offset/Right Controller`
  - `Assets/_EchoRoom/Scenes/MainMenuScene.unity :: XR Origin (XR Rig)/Camera Offset/Left Controller/Menu UI Ray`
  - `Assets/_EchoRoom/Scenes/MainMenuScene.unity :: XR Origin (XR Rig)/Camera Offset/Right Controller/Menu UI Ray`
- **Components/assets/settings:** `PlayerSettings.Android.applicationEntry` is `GameActivity` (`androidApplicationEntry: 2`); `PlayerSettings.Android.buildApkPerCpuArchitecture` is false. `MetaQuestTouchPlusControllerProfile Android` is enabled in `Assets/XR/Settings/OpenXR Package Settings.asset`; `OculusTouchControllerProfile Android`, `MetaXRFeature`, `MetaXRFoveationFeature`, and `MetaXRSubsampledLayout` remain enabled. Both menu controllers retain enabled `ActionBasedController` components. The EventSystem retains enabled `XRUIInputModule` with built-in fallback and enabled `PanelInputConfiguration`. Both menu `XRRayInteractor`s retain `enableUIInteraction=true`. `Assets/Samples/XR Interaction Toolkit/3.3.1/Starter Assets/XRI Default Input Actions.inputactions` retains left/right pointer position and rotation plus `{TriggerButton}` / `{Trigger}` UI Press bindings.
- **Decisions and assumptions:** The headset runtime reported `interaction_profiles/meta/touch_controller_plus`, which did not match the project's previously disabled Touch Plus feature. Enabling the exact profile is the minimum configuration fix; the existing generic `<XRController>{LeftHand|RightHand}` actions already cover the resulting devices, so no scene or action-asset edit was justified. The older Oculus Touch feature remains enabled for Quest 2/controller compatibility. Input System-only remains correct and was not reverted. The Render Graph migration and other unrelated Unity/package-generated working-tree changes were preserved but are not part of this focused fix.
...
- **Goal and resulting behavior:** Port the proven Meta Quest refresh-rate behavior from local `LeafCleanerVR` commit `b381b8315703dd5bd9d1609e8d3308377b31c25d`. Echo Room now uses `XR_FB_display_refresh_rate` when an Android OpenXR session begins, enumerates every runtime-supported rate, selects the highest rate no greater than 90 Hz, and always submits that app-session request even if the headset initially reports 90 Hz. A successful request disables Unity VSync and sets `Application.targetFrameRate` to the accepted selected rate. 72 FPS is retained only as the safe startup/request-failure fallback.
- **Files created:**
  - `Assets/_EchoRoom/Scripts/XR/EchoRoomDisplayRefreshRateFeature.cs`
  - `Assets/_EchoRoom/Scripts/XR/EchoRoomDisplayRefreshRateFeature.cs.meta`
  - `Assets/_EchoRoom/Scripts/Managers/QuestPerformanceTuner.cs.meta`
- **Files modified:**
  - `Assets/_EchoRoom/Scripts/Managers/QuestPerformanceTuner.cs`
  - `Assets/XR/Settings/OpenXR Package Settings.asset`
  - `Docs/PROJECT_MEMORY.md`
- **Files moved or deleted:** None.
- **Unity objects affected:** No scene or prefab object was serialized. `QuestPerformanceTuner` still creates the persistent runtime-only GameObject `Quest Performance Tuner`; it has no scene/prefab asset path to record. The Android OpenXR settings asset gained the enabled `EchoRoomDisplayRefreshRateFeature Android` sub-asset.
- **Components, assets, settings, and dependencies:** Added `EchoRoom.XR.EchoRoomDisplayRefreshRateFeature` with feature ID `com.echoroom.openxr.feature.displayrefreshrate`, company `Echo Room`, extension `XR_FB_display_refresh_rate`, and `BuildTargetGroups = Android` only. The feature resolves `xrEnumerateDisplayRefreshRatesFB`, `xrGetDisplayRefreshRateFB`, and `xrRequestDisplayRefreshRateFB` through `xrGetInstanceProcAddr`. It records the enumerated rates, caps selection at 90 Hz, submits the selected preference on every `OnSessionBegin`, and applies Unity frame pacing only after OpenXR accepts the request. `QuestPerformanceTuner` retains Fixed Foveated Rendering and diagnostics but no longer reads or writes `OVRPlugin.systemDisplayFrequency`, performs an automatic 72 Hz step-down, changes Unity frame pacing, or forces CPU/GPU performance levels; Horizon OS remains responsible for adaptive CPU/GPU levels. Existing Meta XR/OpenXR packages were reused; no dependency was added.
- **Decisions and assumptions:** The LeafCleaner implementation was adapted to game-specific identifiers and restricted to Android because this request targets Meta Quest. The current staged `QuestPerformanceTuner` was directly relevant and had to be updated because its delayed 72 Hz fallback could overwrite a successful 90 Hz OpenXR request. Existing unrelated OpenXR settings, including the already-present uncommitted Touch Plus profile state, were preserved outside the focused commit. The pre-existing unresolved `Docs/PROJECT_MEMORY.md` conflict content was preserved; this entry was appended after it.
...
## 2026-08-05 - TUTORIAL-CONTROLLER-UI-INPUT-001 - Restore tutorial sonar input and controller-anchor tutorial UI

- **Goal and resulting behavior:** Fix the tutorial stopping at the initial sonar instruction, remove the faint hidden timer/objective ghost, and stop the tutorial prompt from following the headset. The tutorial sonar now owns a live Input System binding for the current handedness in addition to the legacy XR feature fallback. In the inspected default `Both` mode, the enabled binding and prompt both resolve to the right-controller `B` button. The tutorial prompt is now reparented to the active gameplay controller and uses a fixed controller-local pose; it no longer derives position or rotation from the headset. The objective panel now combines zero opacity and `display: none` with `visibility: hidden` at rest.
- **Files created, moved, or deleted:** None.
- **Files modified:**
  - `Assets/_EchoRoom/Scripts/ControllerInputSource.cs`
  - `Assets/_EchoRoom/Scripts/Managers/MazeLevelTimer.cs`
  - `Assets/_EchoRoom/Scripts/Tutorial/TutorialDirector.cs`
  - `Docs/PROJECT_MEMORY.md`
- **Unity objects affected and inspected:** No scene or prefab object was serialized by this change. Runtime Play Mode inspection covered:
  - `Assets/_EchoRoom/Scenes/MainScene.unity :: XR Origin (XR Rig)/PlayerController`
  - `Assets/_EchoRoom/Scenes/MainScene.unity :: XR Origin (XR Rig)`
  - `Assets/_EchoRoom/Scenes/MainScene.unity :: Tutorial System`
  - `Assets/_EchoRoom/Scenes/MainScene.unity :: XR Origin (XR Rig)/Camera Offset/Right Controller/Custom Objects Scripts/PingEmiiter`
  - Runtime Play Mode: `Assets/_EchoRoom/Scenes/MainScene.unity :: XR Origin (XR Rig)/Camera Offset/Right Controller/Tutorial UI Toolkit Prompt`
  - Runtime Play Mode: `Assets/_EchoRoom/Scenes/MainScene.unity :: XR Origin (XR Rig)/Camera Offset/Right Controller/Maze Objective Panel`
- **Components, assets, settings, and dependencies:** `ControllerInputSource` now creates and disposes a private `Handed Sonar Ping` `InputAction`, binds it to `HandedInput.SonarPingPath`, and rebuilds it when `EchoRoomSetting.Handedness` changes. The authored `pingAction` remains supported if repaired later, and `XR.InputDevices` remains as a compatibility backstop. Inspection found the scene's serialized Ping reference missing while its locomotion references were valid; the referenced `Assets/Samples/XR Interaction Toolkit/3.3.1/Starter Assets/XRI Default Input Actions.inputactions` contains no `Ping` action. `TutorialDirector` uses `HandednessController.ActiveHandTransform`, reparents the runtime prompt to that transform, and applies local position `(0.000, 0.160, 0.320)`, local Euler `(0.000, 180.000, 0.000)`, and the existing mirrored `0.0005` world scale. The prompt status text is updated at runtime to `CONTROLLER ANCHORED`. `MazeLevelTimer.SetDisplayAlpha` now applies both `DisplayStyle.None` and `Visibility.Hidden` below the visibility threshold. No package or project-setting dependency changed.
- **Decisions and assumptions:** The established handedness mapping was preserved: `Both` uses right-controller `B` for sonar, while one-hand mode uses the active controller's primary face button (`A` on Right or `X` on Left) and the tutorial copy is generated from the same mapping. The missing serialized action reference was not repaired in the sample XRI asset; a runtime-owned action avoids modifying a third-party sample and stays synchronized with the settings event. The prompt uses the controller's full tracked pose by design, matching the request that it follow the controller rather than the headset. The pre-existing unresolved conflict and all existing entries in this journal were preserved; this entry was appended after them.
- **Verification:** Unity 6000.3.8f1 accepted all three C# updates through the registered `script-update-or-create` tool's Roslyn validation and finished with `IsPlaying=false`, `IsCompiling=false`, and `IsUpdating=false`. A controlled tutorial Play Mode run in the pre-existing `Both` preference reported `input.path=<XRController>{RightHand}/secondaryButton`, `input.label=B`, action `Handed Sonar Ping`, `input.enabled=True`, and the same secondary-button binding. The tutorial was at `Ping`; its prompt path was under the Right Controller, `parentIsActiveHand=True`, with the intended local pose. The objective panel reported `alpha=0.000`, `display=None`, `visibility=Hidden`, and `opacity=0.000`. `MainScene` returned to Edit Mode clean. `git diff --check` passed for the three changed scripts. The test runner reported that the project contains no tests; this expected tool response and an earlier inspection-tool parameter error are the only verification-originated Console errors, alongside a pre-existing Android asset-import subtarget mismatch message during Play Mode entry. Editor PlayerPrefs used for the audit were restored to `TutorialRequested=0`, `TutorialStatus=2`, and `Handedness=0`.
- **Known limitations:** The physical Quest controller button event cannot be injected from the desktop Editor, so the next Android headset build must confirm an actual B press advances the tutorial and emits sonar. The controller-local prompt offset and scale were verified numerically, not for comfort in-headset. The project has no automated Unity tests.
- **Follow-up:** Build and run on Quest in `Both`, `Right`, and `Left` handedness modes. Confirm the tutorial prompt names and accepts the mapped sonar button, stays attached to the active controller while the head turns, and the timer/objective is completely absent until explicitly revealed.
## 2026-08-05 - TUTORIAL-CONTROLLER-UI-INPUT-001-CORRECTION - Journal conflict state

- **Correction:** The `TUTORIAL-CONTROLLER-UI-INPUT-001` decisions paragraph says the pre-existing unresolved `Docs/PROJECT_MEMORY.md` conflict was preserved. That was accurate when this task began, but a concurrent `GIT-ORGANIZE-001` project-history cleanup resolved the conflict before final verification. The journal now contains no conflict markers, both former conflict-side entries remain preserved, and this tutorial/input change is an ordinary unstaged modification on top of that resolved state.
- **Files modified:** `Docs/PROJECT_MEMORY.md`.
- **Unity objects affected:** None - documentation correction only.
- **Verification:** `rg` found no `<<<<<<<`, `=======`, or `>>>>>>>` markers in `Docs/PROJECT_MEMORY.md`; focused Git status reports the journal as modified, not unmerged.
## 2026-08-05 - DUAL-CONTROLLER-TRANSITION-PAUSE-001 - Dual-controller stabilization and clean level handoff

- **Goal and resulting behavior:** Resolve the six issues shown in `com.DefaultCompany.EchoRoom-20260805-153858-0.mp4` and the attached controller-HUD image. Prefab-to-prefab level transitions now display the loading UI for at least five real-time seconds, hide it, and only then invoke the synchronous next-level swap, preventing the Level 2 intro and loading UI from rendering together. The tutorial's entity instruction now reads `LOOK AT THE ENTITY` / `Look at the Entity.`. The controller-mounted timer/objective document is no longer created. Settings and runtime input are forced to dual-controller mode, the handedness row is hidden, and stale Left/Right PlayerPrefs are migrated to Both. The tutorial no longer teaches the objective-panel button or hold-B pause. Pause is opened with the Menu button; the obsolete hold-B action is removed, and the Input System plus raw-XR fallback are sampled in the same frame so one physical press produces one toggle.
- **Files created, moved, or deleted:** None.
- **Files modified:**
  - `Assets/_EchoRoom/Scripts/Managers/MazeLevelTimer.cs`
  - `Assets/_EchoRoom/Scripts/Tutorial/TutorialDirector.cs`
  ...
  - `Assets/_EchoRoom/Scripts/UI/VRPauseMenu.cs`
  - `Assets/_EchoRoom/Scripts/UI/VRSettingsPanelController.cs`
  - `Assets/_EchoRoom/Scripts/Utility/HandedInput.cs`
  - `Docs/PROJECT_MEMORY.md`
- **Unity objects affected and inspected:**
  - `Assets/_EchoRoom/Scenes/MainScene.unity :: GameManager`
  - `Assets/_EchoRoom/Scenes/MainScene.unity :: VR Loading Screen`
  ...
  - `Assets/_EchoRoom/Scenes/MainScene.unity :: XR Origin (XR Rig)/Camera Offset/Right Controller`
  - `Assets/_EchoRoom/Scenes/MainScene.unity :: XR Origin (XR Rig)/Camera Offset/Right Controller/Menu UI Ray`
- **Components, assets, settings, and dependencies:** `VRLoadingScreen.CoverPrefabSwap` enforces `MinimumPrefabSwapDisplaySeconds = 5f` even though MainScene's serialized general `minimumDisplayTime` remains 4 seconds, reaches 100%, yields one final loading-only frame, calls `CompleteTransition()`, and then runs the prefab swap. Both loading and level-intro UI still use the shared `UITKOverlay` camera; ordering now prevents overlap instead of relying on sorting. `MazeLevelTimer` remains attached to the Right Controller and continues countdown/failure logic, but `ControllerDisplayEnabled` is false so no `Maze Objective Panel` or reveal action is created. `EchoRoomSettings.Handedness` always resolves/writes `Both`, with a before-scene-load migration of stale preferences; the existing enum and dormant one-hand implementation remain for possible later restoration. `VRSettingsPanelController` hides the `HANDEDNESS` row and does not bind its button. `VRPauseMenu` retains Menu-button bindings for both XR controller layouts and its raw `CommonUsages.menuButton` fallback, removes the hold-B InputAction/event/API, and evaluates the raw fallback unconditionally before combining results. No packages, scene assets, prefabs, InputAction assets, or UI layout assets changed.
- **Root cause:** The loading overlap occurred because `CoverPrefabSwap` invoked `swapAction` almost immediately and then kept the loading UI visible for the rest of its minimum display time, so Level 2 and the loader shared rendered frames. The faint controller text was the intentionally created `Maze Objective Panel`, parented from the `MazeLevelTimer` component authored on the Right Controller. The pause flicker occurred because `PausePressed()` returned `pressed || ControllerPausePressed() || heldPause`; when the Input System saw the Menu press, C# short-circuiting skipped `ControllerPausePressed()`, so its `controllerWasPressed` latch stayed false. On the following held frame, the raw XR path reported a second rising edge and toggled the newly opened menu closed.
- **Decisions and assumptions:** Dual-controller mode is a temporary product decision, so compatibility types and one-hand branches were retained but made unreachable through settings rather than destructively deleting the architecture. The timer continues to govern failure with no controller visual readout, matching the request to remove the unexpected panel. The existing loading-screen presentation was reused; no new fake scene or asset was added. Existing unrelated worktree changes and untracked Lever assets were preserved.
- **Verification:** The supplied 95.067-second video was sampled locally; frames around 1:21 confirmed the Level 2 title appeared while `TUNING SIGNAL` was still visible. Unity 6000.3.8f1 accepted every C# update through the registered Roslyn-validating script tool and finished with `IsPlaying=false`, `IsCompiling=false`, and `IsUpdating=false`; `MainScene` remained loaded and clean. A controlled Play Mode audit migrated stored handedness `Right (2)` to `Both (0)`, reported `HandedInput.IsSingleHand=False`, settings handedness row `display=None`, zero live `Maze Objective Panel` objects, no hold-pause field/method, and exact entity copy `LOOK AT THE ENTITY` / `THE ENTITY\nLook at the Entity.`. A harmless no-op `CoverPrefabSwap` audit invoked its callback after 5.005 seconds with `IsVisible=False` inside the callback and `IsTransitioning=False` after completion. `git diff --check` passed. Unity's test runner reported that the project contains no tests. Current-run logs contained no project error or exception; the only audit exception/error came from the MCP console-clear tool failing to delete its own locked log file, and Play Mode also emitted the expected Quest Link/OpenXR warning on desktop.
- **Known limitations:** A physical Quest controller Menu press cannot be injected from the desktop Editor, so the fixed edge behavior still needs one headset confirmation. The full Level 1 exit was not replayed automatically; the shared transition helper was timed directly with a no-op swap. With the controller panel removed, the countdown remains gameplay logic but has no visual timer/objective presentation.
- **Follow-up:** In the next Quest build, complete Level 1 and confirm five seconds of loading-only presentation followed by Level 2 with no shared frame. Tap and hold the left Menu button once each and confirm the pause menu opens once, remains stable, and closes only on a new press. Replay the tutorial to confirm no objective-panel or hold-B lesson appears and the final entity copy reads `LOOK AT THE ENTITY`.
## 2026-08-05 - TIMER-PANEL-A-RESTORE-001 - Restore the Right-A timer/objective peek

- **Correction to `DUAL-CONTROLLER-TRANSITION-PAUSE-001`:** The prior entry recorded the timer/objective panel and reveal action as intentionally disabled. That interpretation was incorrect. The intended behavior is for the panel to remain completely hidden at rest and appear temporarily when the player presses Right A.
- **Goal and resulting behavior:** Restore the maze timer/objective panel and its input without restoring the faint always-visible ghost. `MazeLevelTimer` now creates the runtime panel and enables the reveal action again. In dual-controller play, Right A shows the panel for the existing three-second reveal window; once the window expires it returns to `display: none`, `visibility: hidden`, and zero opacity.
- **Files created, moved, or deleted:** None.
- **Files modified:**
  - `Assets/_EchoRoom/Scripts/Managers/MazeLevelTimer.cs`
  - `Docs/PROJECT_MEMORY.md`
- **Unity objects affected and inspected:**
  - `Assets/_EchoRoom/Scenes/MainScene.unity :: XR Origin (XR Rig)/Camera Offset/Right Controller`
- **Components, assets, settings, and dependencies:** `MazeLevelTimer.ControllerDisplayEnabled` is restored to true. The existing `Reveal Maze Timer` runtime `InputAction` binds `HandedInput.ObjectivePanelPath`, which resolves to `<XRController>{RightHand}/primaryButton` in the forced `Both` preference; the existing Gamepad South and keyboard T audit bindings remain. The earlier `SetDisplayAlpha` hard-hide behavior is unchanged. Existing level-start flashes, puzzle-progress flashes, and low-time warning reveals are also unchanged.
- **Root cause:** The previous stabilization pass set `ControllerDisplayEnabled` to false. `Awake()` therefore skipped both `CreateRevealAction()` and `EnsureDisplay()`, so pressing A had no live action or panel to reveal.
- **Decisions and assumptions:** The screenshot was interpreted as the unwanted zero-alpha ghost, not a request to delete the timer feature. The corrected contract is hidden at rest plus temporary Right-A reveal. No UI assets, scene serialization, offsets, timings, countdown rules, or objective calculations changed.
- **Verification:** Unity 6000.3.8f1 accepted the C# correction through the registered Roslyn-validating script tool and completed compilation. In controlled MainScene Play Mode, the timer reported `running=True`; the reveal action was enabled with bindings `<XRController>{RightHand}/primaryButton`, `<Gamepad>/buttonSouth`, and `<Keyboard>/t`. Runtime visibility audit reported `None/Hidden/0.000` at rest, `Flex/Visible/1.000` after a simulated reveal press, and `None/Hidden/0.000` after the three-second timeout. `git diff --check` passed. The Unity test runner reported that the project contains no tests.
- **Known limitations:** The physical Quest A-button event cannot be injected from the desktop Editor; the live action binding and reveal state machine were verified independently and still need one on-headset press confirmation.
- **Follow-up:** In the next Quest build, press Right A during a maze and confirm the panel appears clearly for approximately three seconds, then becomes completely invisible again.
## 2026-08-05 - TIMER-PANEL-GHOST-A-INPUT-002 - Eliminate rebuilt objective ghost and harden Right-A reveal

- **Goal and resulting behavior:** Remove the dark TIME / objective / progress text that remained visible on level geometry while preserving the intended temporary timer panel. The objective UIDocument is now fully inactive at rest, is rebuilt and restyled before a reveal, and is fully deactivated again when its reveal window expires. Right-controller A now drives the reveal through both the Input System action and a direct XR primaryButton rising-edge fallback.
- **Files created, moved, or deleted:** None.
- **Files modified:**
  - Assets/_EchoRoom/Scripts/Managers/MazeLevelTimer.cs
  - Docs/PROJECT_MEMORY.md
- **Unity objects affected and inspected:**
  - Assets/_EchoRoom/Scenes/MainScene.unity :: XR Origin (XR Rig)/Camera Offset/Right Controller
  - Runtime-only child: Assets/_EchoRoom/Scenes/MainScene.unity :: XR Origin (XR Rig)/Camera Offset/Right Controller/Maze Objective Panel
- **Components, assets, settings, and dependencies:** MazeLevelTimer retains its runtime UIDocument, private cloned PanelSettings, Resources/UI/VRObjectivePanel.uxml, Resources/UI/VRObjectivePanelStyles.uss, and <XRController>{RightHand}/primaryButton Input System binding. Its hidden state now deactivates the document GameObject and clears stale VisualElement references. Each activation queries the new visual tree, reapplies the objective stylesheet and picking settings, refreshes timer/objective text, and then applies the requested opacity. The direct XR fallback reads UnityEngine.XR.CommonUsages.primaryButton from XRNode.RightHand and updates its edge latch every frame before combining with the Input System result. The warning AudioSource now lives on the always-active Right Controller timer object so document deactivation cannot suppress warning audio. No scene, prefab, package, InputAction asset, UXML, USS, or project-setting asset changed.
- **Root cause:** UI Toolkit can discard and recreate a world-space UIDocument visual tree. The previous code kept references to the old tree and hid only its nested root with opacity/display/visibility. If the document rebuilt afterward, the new tree used raw UXML defaults: transparent containers and black label text, matching the faint text shown on the brick wall, while the script continued reporting the stale old root as hidden. Separately, the reveal depended only on InputAction.WasPressedThisFrame(), which was not reliably reporting the Quest Right-A press in the headset build.
- **Decisions and assumptions:** Full document deactivation is the authoritative hidden state because an inactive document cannot render a replacement tree. Rebinding on activation is required because UIDocument.rootVisualElement becomes null while inactive and is recreated when re-enabled. The project is currently forced to dual-controller play, so the raw fallback intentionally targets Right A. The Input System binding, Gamepad South, and keyboard T paths remain for compatibility and desktop verification.
- **Verification:** Unity 6000.3.8f1 accepted the script through the registered Roslyn-validating script-update-or-create tool and finished with IsPlaying=false, IsCompiling=false, and IsUpdating=false. MainScene Play Mode inspection confirmed the authored timer path above. At rest the runtime document reported activeSelf=False, rootVisualElement=null, and the cached panel root null. A controlled reveal rebuilt a current root with one objective stylesheet and reported Flex/Visible/1.000; hiding reported activeSelf=False and rootVisualElement=null. A second rebuild reported Flex/Visible/0.500, confirming rest-to-reveal works repeatedly rather than only once. The actual TickTimer(..., revealPressed:true) path reported Flex/Visible/1.000, then returned to an inactive/null-root state after the reveal window. The action was enabled with <XRController>{RightHand}/primaryButton, <Gamepad>/buttonSouth, and <Keyboard>/t; the raw XR fallback method was present; the warning audio source was active on the timer object. git diff --check passed for the timer script. The project contains no Unity tests, so tests-run returned the expected No tests found; there were no runtime exceptions, and the only final Console error was the test tool reporting that absence.
- **Known limitations and follow-up:** Desktop Play Mode cannot inject a physical Quest controller feature value. Build to Quest and confirm that Right A reveals the fully styled panel for approximately three seconds, that repeated presses work, and that no dark label-only ghost appears before or after the reveal.

## 2026-08-05 - PRIVACY-CONSENT-001 - First-run privacy acceptance and permanent policy link

- **Goal and resulting behavior:** Add the one-time privacy-policy acceptance requested for the Meta Store release and keep the policy reachable afterward. On a fresh install/profile, MainMenuScene now opens on a blocking Privacy Policy screen with REVIEW PRIVACY POLICY and I ACCEPT. Acceptance is saved locally and later launches go directly to the normal menu. The normal menu always includes a separate PRIVACY POLICY button. Both review buttons open https://w42dev.netlify.app/ through the platform browser.
- **Files created, moved, or deleted:** None.
- **Files modified:**
  - Assets/_EchoRoom/Resources/UI/VRMenu.uxml
  - Assets/_EchoRoom/Resources/UI/VRMenu.uss
...
- **Unity objects affected and inspected:**
  - Assets/_EchoRoom/Scenes/MainMenuScene.unity :: Main Menu
- **Components, assets, settings, and dependencies:** The existing UIDocument and EchoRoom.UI.VRMainMenu remain on Main Menu; no scene serialization or package dependency changed. VRMenu.uxml now contains privacy-screen, privacy-review-button, privacy-accept-button, and the permanent privacy-policy-button. VRMenu.uss adds headset-readable styling for the acceptance screen and compact permanent link. VRMainMenu binds both policy buttons to Application.OpenURL, gates ShowStartPanel() until consent exists, and persists the versioned PlayerPrefs key EchoRoom.PrivacyPolicy.Accepted.v1 with value 1.
- **Decisions and assumptions:** Consent is one-time per application data/profile, matching the requested behavior; clearing application data shows it again. The key is versioned so a future materially revised policy can deliberately require fresh acceptance by moving to a new version. The policy is displayed in the system browser rather than duplicated in-game, keeping the hosted page authoritative. The shared menu layout contains the hidden privacy elements, while the dedicated MainMenuScene controller owns the gate and link callbacks.
- **Verification:** Unity 6000.3.8f1 imported all three changed assets and finished with IsPlaying=false, IsCompiling=false, and IsUpdating=false. A controlled fresh-preference Play Mode run reported privacy=Flex, start=None, both acceptance buttons present/enabled, and stored preference 0. Invoking the actual private acceptance handler changed the preference to 1, hid the privacy screen, and showed the start screen. The acceptance button bounds were fully inside the privacy screen bounds. A full Play Mode restart with preference 1 reported privacy=None and start=Flex, confirming persistence. The permanent button was enabled, labeled PRIVACY POLICY, and fully inside the start-screen bounds. The original Editor preference state (key absent) was restored after testing. Unity script readback confirmed the final controller source, git diff --check passed for the three implementation files, and the hosted URL returned HTTP 200. The Unity test runner reported that the project contains no tests. The only recent Console errors were generated by an earlier invalid empty Unity-tool enablement probe, not by project compilation or runtime behavior.
- **Known limitations:** The system browser handoff and controller clicks were not exercised on a physical Quest headset. Acceptance is stored with PlayerPrefs, so uninstalling the app, clearing its data, or using a separate device/profile requires acceptance again.
- **Follow-up:** In the release-candidate Quest build, use a fresh install to press REVIEW PRIVACY POLICY, return to the game, accept, restart the app, and confirm the gate stays dismissed while the permanent main-menu button still opens the policy.

## 2026-08-05 - PRIVACY-CONSENT-001-CORRECTION - Final Console verification note

- **Correction:** The PRIVACY-CONSENT-001 verification paragraph says the only recent Console errors were from the earlier empty tool-enablement probe. The final three-minute Console query instead contained only the Unity test runner reporting No tests found. No project compilation or runtime privacy-flow error or exception was present.
- **Files modified:** Docs/PROJECT_MEMORY.md.
- **Unity objects affected:** None - documentation correction only.
- **Verification:** MainMenuScene remained loaded, valid, clean, and at build index 0 after Play Mode verification.

## 2026-08-05 - MAINMENU-VERSION-LABEL-001 - Dynamic app version on the main panel

- **Goal and resulting behavior:** Show the current application version on the main menu. The existing world-space panel now displays VERSION 1.0 in its bottom-right corner, opposite the bottom-left ECHO NETWORK status. The text is populated from Application.version at runtime, so future Unity bundleVersion changes automatically appear without editing the menu layout.
- **Files created, moved, or deleted:** None.
- **Files modified:**
  - Assets/_EchoRoom/Resources/UI/VRMenu.uxml
  - Assets/_EchoRoom/Resources/UI/VRMenu.uss
...
- **Unity objects affected and inspected:**
  - Assets/_EchoRoom/Scenes/MainMenuScene.unity :: Main Menu
- **Components, assets, settings, and dependencies:** The existing UIDocument and EchoRoom.UI.VRMainMenu remain unchanged in scene serialization. VRMenu.uxml adds the non-interactive app-version-label under the panel shell, VRMenu.uss anchors it 23 pixels from the right and 15 pixels from the bottom, and VRMainMenu sets its text to VERSION plus Application.version during Awake. ProjectSettings/ProjectSettings.asset currently supplies bundleVersion 1.0 and was inspected but not modified. No package, scene, prefab, or project setting changed.
- **Decisions and assumptions:** The label mirrors the muted styling and baseline of the existing network status, keeping release metadata visible without competing with menu actions. It lives outside individual screen containers so it remains a consistent panel footer, including on the first-run privacy screen.
- **Verification:** Unity 6000.3.8f1 imported and compiled the changed assets with IsPlaying=false, IsCompiling=false, and IsUpdating=false before the runtime check. In controlled MainMenuScene Play Mode, Application.version was 1.0 and app-version-label reported text VERSION 1.0 with display Flex. Its bounds were fully inside the 650 by 530 panel shell, separated from the bottom-left status, and aligned on the same footer baseline. The temporary privacy-consent preference used to expose the normal start screen was restored to its original absent state. MainMenuScene returned to Edit Mode valid and clean. Focused git diff --check passed. The Unity test runner reported No tests found; this expected response was the only recent Console error, with no project compilation or runtime error.
- **Known limitations:** Final perceived size and stereo readability still need a physical Quest headset check. The displayed value follows Unity bundleVersion, not Android bundleVersionCode.
- **Follow-up:** In the next Quest build, confirm VERSION 1.0 is comfortably readable in the lower-right corner and update bundleVersion before each store release.

## 2026-08-05 — FIRST-RUN-PRIVACY-ORDER-001 — Privacy acceptance before tutorial choice

- Goal: Ensure a fresh installation presents the privacy policy before the tutorial Play/Skip choice.
- Resulting behavior: The tutorial first-run coroutine now waits until privacy consent is recorded. Fresh installs see Privacy Policy -> Tutorial/Skip -> main menu; returning users with consent continue directly to the tutorial decision or normal menu flow.
- Files modified:
  - Assets/_EchoRoom/Scripts/UI/VRMainMenu.cs
  - Assets/_EchoRoom/Scripts/Tutorial/TutorialDirector.cs
  - Docs/PROJECT_MEMORY.md
- Files created, moved, or deleted: None.
- Unity objects inspected/affected:
  - Assets/_EchoRoom/Scenes/MainMenuScene.unity :: Main Menu — inspected in the Unity Editor; its UIDocument and EchoRoom.UI.VRMainMenu provide the privacy screen and acceptance state. No scene serialization change.
  - Assets/_EchoRoom/Scenes/MainMenuScene.unity :: Tutorial First Run Prompt — inspected at runtime in the Unity Editor; runtime-only root object with TutorialMenuPrompt now waits before adding tutorial-offer-screen. No scene serialization change.
- Important references and settings:
  - VRMainMenu.IsPrivacyPolicyAccepted exposes the existing EchoRoom.PrivacyPolicy.Accepted.v1 PlayerPrefs state as a read-only public static property.
  - TutorialMenuPrompt.Start yields while privacy is unaccepted, then adds the replay button and evaluates TutorialProgress.ShouldOffer.
  - Tutorial keys remain EchoRoom.Tutorial.Status.v1 and EchoRoom.Tutorial.Requested; their semantics were not changed.
- Decisions and assumptions: Privacy acceptance is a hard prerequisite for the tutorial choice. The waiting prompt host remains alive without creating an overlay, so it does not cover or intercept the privacy panel. Accepting privacy releases the wait on the next frame. Existing accepted installations are unchanged.
- Known limitations/follow-up: A final physical Quest/headset first-install check is still recommended for controller focus and Store build behavior. The project currently contains no discoverable EditMode tests.
- Verification: Unity ForceSynchronousImport completed. A controlled Play Mode run with all three first-run keys cleared showed privacy-screen=Flex, no tutorial-offer-screen, and the runtime Tutorial First Run Prompt present. Invoking privacy acceptance then showed privacy-screen=None and tutorial-offer-screen=Flex while tutorial status remained undecided. The editor returned to clean Edit Mode, and original PlayerPrefs values (privacy=1, tutorial status=1, requested=0) were restored. Source checks found each new gate exactly once with no trailing whitespace. EditMode test runner reported No tests found; recent console errors were limited to tool/test-harness diagnostics, with no project compiler or runtime error observed.

## 2026-08-05 - MAZE5-LEVEL-COMPLETE-001 - Reachable final exit and level-completed return flow

- **Goal and resulting behavior:** Make Maze 5 end at its authored exit and use the requested presentation. Reaching the final hallway now activates the final exit before the sealed door, displays `LEVEL COMPLETED` for the existing five-second completion interval, changes to the loading presentation, asynchronously loads `MainMenuScene`, and returns the player to the main menu.
- **Files created, moved, or deleted:** None.
- **Files modified:**
  - `Assets/_EchoRoom/Prefabs/Level prefabs/Maze_5x5_E.prefab`
  - `Assets/_EchoRoom/Resources/UI/VRLoadingScreen.uxml`
  ...
  - `Docs/PROJECT_MEMORY.md`
- **Unity objects affected and inspected:**
  - `Assets/_EchoRoom/Prefabs/Level prefabs/Maze_5x5_E.prefab :: Maze_5x5_E/Exit_Hallway/ExitCheckpoint`
  - `Assets/_EchoRoom/Prefabs/Level prefabs/Maze_5x5_E.prefab :: Maze_5x5_E/Exit_Hallway/Exit_Door` (inspected; unchanged)
  - `Assets/_EchoRoom/Prefabs/Level prefabs/Maze_5x5_E.prefab :: Maze_5x5_E/Exit_Hallway/Hallway_Floor` (inspected; unchanged)
  - `Assets/_EchoRoom/Scenes/MainScene.unity :: GameManager` (runtime behavior only; no scene serialization change)
  - `Assets/_EchoRoom/Scenes/MainMenuScene.unity :: VR Loading Screen` (inspected in Edit Mode and exercised in Play Mode; no scene serialization change)
- **Components, assets, settings, and dependencies:** `ExitCheckpoint` keeps its enabled trigger `BoxCollider` size `(1.25, 2.5, 2.1)` and `LevelExitTrigger.playerTag = Player`. Its local position changes from `(-26.795597, 1.25, -11.1955385)` behind the door to `(-24.4, 1.25, -11.1955385)` on the hallway side, and `LevelExitTrigger.requiredPuzzle` changes from Maze E's root `EchoPuzzleController` to null. `GameManager` calls `VRLoadingScreen.ShowLevelCompletedThenLoadScene` after saving the final puzzle. The repurposed completion panel displays `LEVEL COMPLETED`, `FINAL MAZE COMPLETE`, and `RETURNING TO MAIN MENU`; the existing serialized five-second completion duration, async scene loading, loading progress, minimum display handling, persistent `UIDocument`, and destination-owned `CompleteTransition()` remain unchanged. No package, build setting, scene asset, material, or input dependency changed.
- **Decisions and assumptions:** The sealed exit door remains the visual destination and physical boundary. The trigger was moved immediately in front of it instead of inventing a new door-opening puzzle because Maze E has no lever/button targets and its previous puzzle requirement could never solve. Existing UXML element names and CSS classes that contain `thank-you` were retained as internal compatibility identifiers; all player-facing thank-you copy was removed. Existing unrelated worktree changes, including the prior loading-transition timing fix in `VRLoadingScreen`, were preserved.
- **Verification:** Unity 6000.3.8f1 saved and re-read the prefab with `ExitCheckpoint` world position `(-24.65, 1.25, -11.1955385)` and a null puzzle reference, while the door remained at world X `-25.85` and the hallway floor extended to approximately X `-25.90`. Both C# scripts passed the registered Roslyn validation after the final caller/callee update, AssetDatabase synchronous refresh completed, and the Editor finished in clean Edit Mode with `IsCompiling=false` and `IsUpdating=false`. A controlled Play Mode audit reported `title=LEVEL COMPLETED`, `completionDisplay=Flex`, `loadingDisplay=None`, `busy=True`, and `transitioning=True`; the follow-up reported the completion hidden, loading presentation selected, active scene `MainMenuScene`, and the transition complete. Focused `git diff --check` passed. The project has no discoverable EditMode tests, so the test runner returned `No tests found`; the final one-minute Console query contained no errors or exceptions.
- **Known limitations and follow-up:** The entire Maze 5 route and trigger entry were not replayed with a physical Quest headset. In the next build, walk to the final door, confirm the trigger activates before collision with the door, verify the completion card remains comfortably readable, and confirm the loading-to-menu handoff in-headset.

## 2026-08-05 - MAINMENU-OFFICIAL-TITLE-001 - Official game title in main menu

- **Goal and resulting behavior:** Update the player-facing game title to the official name. The main menu now displays `Echo Room VR : Escape the Dark` instead of `ECHO ROOM`.
- **Files created, moved, or deleted:** None.
- **Files modified:**
  - `Assets/_EchoRoom/Resources/UI/VRMenu.uxml`
  - `Assets/_EchoRoom/Resources/UI/VRMenu.uss`
...
- **Unity objects affected and inspected:**
  - `Assets/_EchoRoom/Scenes/MainMenuScene.unity :: Main Menu`
- **Components, assets, settings, and dependencies:** The existing `UnityEngine.UIElements.UIDocument` and `EchoRoom.UI.VRMainMenu` on `Main Menu` remain unchanged in scene serialization. The UIDocument continues to use `Assets/_EchoRoom/Resources/UI/VRMenu.uxml`, which now contains the exact official title in its `main-title` label. `Assets/_EchoRoom/Resources/UI/VRMenu.uss` changes only `.main-title` from 50px / 13px letter spacing to 34px / 5px so the longer title fits the existing panel. No scene, prefab, script, package, or project setting was modified by this change.
- **Decisions and assumptions:** The supplied capitalization, spacing, and colon are treated as authoritative and reproduced exactly. The typography adjustment is scoped to the main title; section titles and other screens retain their existing styles. Existing unrelated worktree changes, including the already-updated Player Settings product name and new art assets, were preserved.
- **Verification:** Unity 6000.3.8f1 synchronously imported both UI assets. Direct Editor inspection confirmed `MainMenuScene :: Main Menu` is active and backed by `VRMenu.uxml`. The resolved `main-title` reported text `Echo Room VR : Escape the Dark`, display `Flex`, font size 34, letter spacing 5, width 572, and panel width 650, confirming it fits inside the panel. `MainMenuScene` was clean during inspection, and the previously active `MainScene` was restored clean afterward. Focused `git diff --check` passed. Console errors observed during the session were limited to failed inspection/tool attempts caused by a stale GameObject reference, a line-ending guard, and a scene-open request while Play Mode was active; the corrected import and final title verification succeeded with no project compilation error.
- **Known limitations and follow-up:** The fit was verified from the live UI Toolkit layout in the Editor, not through a physical Quest headset. Confirm final stereo readability in the next headset build. On a fresh profile the existing privacy-consent screen still appears before the normal start screen, so the title is shown after consent as designed.

## 2026-08-05 - TIMER-PANEL-RESUME-003 - Preserve timer reveal across Quest suspend

- **Goal and resulting behavior:** Keep the Right-A timer/objective peek working after the Quest application is minimized and resumed. A temporary XR controller shutdown no longer permanently stops the maze timer. Application pause/focus loss hides the panel and disables its reveal action; focus/resume synchronizes the raw A-button latch, re-enables the Input System action, and leaves the active countdown ready for the next A press.
- **Files created, moved, or deleted:** None.
- **Files modified:**
  - Assets/_EchoRoom/Scripts/Managers/MazeLevelTimer.cs
  - Docs/PROJECT_MEMORY.md
- **Unity objects affected and inspected:**
  - Assets/_EchoRoom/Scenes/MainScene.unity :: XR Origin (XR Rig)/Camera Offset/Right Controller
  - Runtime-only child: Assets/_EchoRoom/Scenes/MainScene.unity :: XR Origin (XR Rig)/Camera Offset/Right Controller/Maze Objective Panel
- **Components, assets, settings, and dependencies:** MazeLevelTimer.OnDisable still disables the runtime reveal action, unsubscribes events, stops presentation coroutines/audio, and hard-hides the UIDocument, but no longer changes the gameplay-owned timerRunning flag. OnEnable synchronizes the raw Right-A state and restarts a pending pre-countdown coroutine when necessary. OnApplicationPause and OnApplicationFocus explicitly suspend/restore the display input. The existing <XRController>{RightHand}/primaryButton binding, direct UnityEngine.XR.CommonUsages.primaryButton fallback, private PanelSettings, UXML/USS assets, reveal duration, and countdown logic remain unchanged. No scene, prefab, package, InputAction asset, UI asset, or project setting changed.
- **Root cause:** The timer component is authored on the Right Controller. Quest temporarily disables and later re-enables that controller object while the application/XR session is suspended. The previous OnDisable() treated this transient lifecycle event like a permanent gameplay shutdown and set timerRunning=false. After resume, Update() returned before calling RevealPressed(), so neither the Input System binding nor raw XR fallback could observe A.
- **Decisions and assumptions:** Only explicit gameplay paths such as level completion, tutorial completion, or timer expiry may stop the countdown. Component/controller disable is treated as temporary. Resume hides any pre-suspend panel instead of restoring a partially elapsed reveal, and the raw latch is initialized from the current physical button state so returning while A is already held cannot create a false rising edge. Both application pause and focus callbacks are handled because Android/Quest lifecycle ordering can vary.
- **Verification:** Before the change, a controlled MainScene Play Mode controller disable/enable reported before=True, disabled=False, and after=False, reproducing the permanent failure. After the change the same lifecycle reported duringDisable=True and afterEnable=True; a simulated reveal immediately produced Flex/Visible/1.000. A controlled application pause reported runningPaused=True and actionPaused=False; resume reported runningResumed=True, actionResumed=True, and the next reveal again produced Flex/Visible/1.000. Unity accepted the script through the registered Roslyn-validating writer and returned to clean MainScene Edit Mode with IsCompiling=false and IsUpdating=false. Focused git diff --check passed. The project contains no discoverable EditMode tests; recent Console diagnostics were from unrelated concurrent UI tooling, the expected no-tests result, and an inspection scene-open timing error, with no project compilation or timer runtime exception.
- **Known limitations and follow-up:** Desktop Play Mode reproduces the controller and application callbacks but cannot perform an actual Android Home-button suspend. In the next Quest build, minimize during an active maze, resume, release A if it was held, then press Right A and confirm the panel appears for approximately three seconds while the countdown continues.


## 2026-08-10 - VRC-SEATED-TURN-001 - Restore seated stick turning for Quest review

- **Goal and resulting behavior:** Address Meta VRC.Quest.Tracking.1 while retaining Sitting support. The gameplay rig now uses the current XR Interaction Toolkit mediator-based snap and continuous turn providers, so a seated player can rotate with the right stick without physically turning. The existing turn preference still selects snap or smooth turning at runtime, and handedness remapping still swaps snap-turn input when enabled.
- **Files modified:**
  - `Assets/_EchoRoom/Prefabs/XR Origin (XR Rig).prefab`
  - `Assets/_EchoRoom/Scenes/MainScene.unity`
  - `Assets/_EchoRoom/Scripts/Controller/HandednessController.cs`
  - `Assets/_EchoRoom/Scripts/Controller/TurnSettingsController.cs`
  - `ProjectSettings/ProjectSettings.asset`
  - `Docs/PROJECT_MEMORY.md`
- **Files created, moved, or deleted:** No retained files were created, moved, or deleted. The failed Android build temporarily generated and then removed `Assets/Resources/PerformanceTestRunInfo.json`, `Assets/Resources/PerformanceTestRunSettings.json`, `Assets/StreamingAssets/RuntimeActionBindings.json`, and `Assets/packages-merged-link/link.xml`. No APK was produced.
- **Unity objects affected and inspected:**
  - `Assets/_EchoRoom/Prefabs/XR Origin (XR Rig).prefab :: XR Origin (XR Rig)`
  - `Assets/_EchoRoom/Prefabs/XR Origin (XR Rig).prefab :: XR Origin (XR Rig)/Locomotion System/Turn`
  - `Assets/_EchoRoom/Scenes/MainScene.unity :: XR Origin (XR Rig)`
  - `Assets/_EchoRoom/Scenes/MainScene.unity :: XR Origin (XR Rig)/Locomotion System/Turn`
  - `Assets/_EchoRoom/Scenes/MainMenuScene.unity :: XR Origin (XR Rig)` (inspected; unchanged)
  - `Assets/_EchoRoom/Scenes/MainMenuScene.unity :: XR Origin (XR Rig)/Locomotion System/Turn` (inspected; unchanged and inactive because main-menu movement remains locked)
- **Components, assets, settings, and dependencies:** Replaced deprecated `ActionBasedSnapTurnProvider` and `ActionBasedContinuousTurnProvider` components with `UnityEngine.XR.Interaction.Toolkit.Locomotion.Turning.SnapTurnProvider` and `ContinuousTurnProvider` from XR Interaction Toolkit 3.3.1. Both reference the root `LocomotionMediator`/`XRBodyTransformer`. Snap settings remain 45 degrees with 0.5-second debounce; smooth speed remains 60 degrees per second. Left/right snap and smooth actions remain the corresponding actions in `XRI Default Input Actions.inputactions`, bound to each controller's `Primary2DAxis`. `TurnSettingsController` and `HandednessController` now resolve the current provider types. Release metadata changed from bundle version 1.0 / Android code 1 to bundle version 1.1 / Android code 2. Meta XR SDK 77, Unity OpenXR 1.16.1, ARM64-only APK-per-architecture configuration, enabled build scenes, product identity, and custom keystore/alias configuration were otherwise preserved.
- **Root cause:** The authored legacy snap provider ran beside a current `LocomotionMediator` but had no legacy `LocomotionSystem`. XR Interaction Toolkit 3.3.1's obsolete provider path therefore returned `CanBeginLocomotion=false` even though both right- and left-stick InputActions were enabled and resolved to Quest controls. Input was present, but the provider could never begin a turn.
- **Decisions and assumptions:** Sitting remains an advertised play mode because stick turning removes the reviewer's need to physically pivot. Standing and room-scale use remain possible; this change does not force a single tracking-origin mode. Both turn providers stay serialized disabled and the existing settings controller enables exactly the selected mode at runtime. Main-menu locomotion remains intentionally locked. No claim is made that Meta review is guaranteed; the change directly addresses the reproduced blocker.
- **Verification:** In controlled MainScene Play Mode, the new snap provider was enabled with a valid `LocomotionMediator`, and its right-turn action was enabled with one resolved control. Injecting right-turn input rotated rig yaw exactly from 270 to 315 degrees. A controlled smooth-turn check rotated yaw from 315 to 8.04376 degrees at a temporary 5 degrees/second, after which the authored 60 degrees/second setting and snap-default selection were restored. Prefab readback confirmed the mediator and all four InputAction references, and MainScene readback confirmed `HandednessController` references the new snap provider. MainMenuScene remained clean with locomotion locked. No deprecated turn-provider references remain under `Assets/_EchoRoom`. Unity accepted and compiled both scripts. The Unity test runner reported `No tests found`, consistent with the project having no discoverable EditMode tests.
- **Build status and blocker:** Two automated retries targeted `Builds/EchoRoomVR_v1.1_b2.apk`; both stopped before Gradle because Unity's custom keystore and key-alias passwords were not loaded in the Editor session. The final report was `Failed`, zero bytes, one warning, and one error with `Unable to sign the application; please provide passwords!`. The signing dialog was dismissed and the build-generated temporary files were removed. No password or other secret was read, requested, or recorded.
- **Known limitations and follow-up:** Runtime turning was verified with desktop Unity input injection, not on a physical Quest while seated. In Unity Android Publishing Settings, enter the existing keystore password and key-alias password locally, then rerun the version 1.1 (code 2) ARM64 APK build. Install that APK on Quest, verify snap and smooth right-stick turning while seated without physical pivoting, then upload it for Meta review.
